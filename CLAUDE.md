...
The four dual-context commands, plus the two single-context, host-app-only commands `createATOM`/`createApp`, are all now thin wrappers over `thecore_generators` — see the next section — and so is `checkPractices` (see its own section further below). `createATOM` no longer does any file placement of its own at all; that decision lives entirely in `thecore_generators` for it. `createApp` (thecore_code_extension#41) was the last command to delegate, shelling out to the App application template (`rails new -m <app_template.rb URL>`, ADR 0005/0007 in the `thecore` repo) once its non-interactive mode (thecore_generators#23) shipped — see its own subsection below — but, unlike the others, it still writes real files of its own after the template runs: `config/sidekiq.yml`, the `version` file, a `development.rb` patch, a custom `.gitignore` (via `ctx.write.gitignoreFile`), `vendor/custombuilds`/`vendor/deploytargets`, and its own git init/commit. The App template is scoped to Gemfile/CI/devcontainer/CLAUDE.md/the installer chain only (ADR 0005) — it was never going to absorb *everything* `createApp.js` did, unlike `createATOM`'s much more complete handoff to `thecore:atom`. See `createApp`'s own subsection below for exactly what stayed JS-side and why.
...
Because `addModel`/`addMigration`/`addRootAction`/`addMemberAction`/`createATOM`/`checkPractices` now trust `rails` completely, a host app whose `Gemfile` doesn't actually depend on `thecore_generators` gets a **silent** regression (for `model`/`migration`) or an outright failure (`thecore:root_action`/`thecore:member_action`/`thecore:atom`/`thecore:check_practices` simply don't exist as namespaces/tasks) — there is no clear "you're missing a dependency" signal either way. All six commands guard against this after their existing context guard checks (`hasGemspec`/`railsAppValid`) but *before* collecting the name (and definition, or the six ATOM-metadata prompts) or running the audit, so a dismissed prompt doesn't waste typing (`createApp` is the one command among the seven delegated ones that never needed this guard at all — it bootstraps a brand-new app with no existing Gemfile to check, and the App template it delegates to adds its own `thecore_generators` dependency unconditionally):
...
- On a failed check, `confirmAndAddThecoreGenerators(ctx, gemfilePath)` (`libs/thecoreGeneratorsGuard.js`) shows a `vscode.window.showWarningMessage` explaining the silent-fallback risk, with a single **"Add & Bundle Install"** action button.
  - **Dismissed/cancelled** (any response other than that exact button, including pressing Escape) — the function returns `false`, all six commands `return` immediately, and no `rails`/`bundle` command of the caller's own ever runs.
...
Regression check: a workspace whose Gemfile already has `thecore_generators` never triggers the warning at all — `ctx.check.hasThecoreGenerators` is `ok: true` and all six commands proceed exactly as before this guard existed.

### `createApp` — thin wrapper over the App application template (thecore_code_extension#41)

Unlike every other delegated command, `createApp` only delegates *part* of what it does — the App template (ADR 0005/0007) is scoped to the Gemfile stack, both CI files, devcontainer/CLAUDE.md assets, `vendor/submodules`/`vendor/external` placeholders, and the installer chain, never to *everything* `createApp.js` used to hand-roll. After `sudo chown -R vscode:vscode .` (unchanged, unrelated to delegation), it shells out to `rails new . --database=postgresql --asset-pipeline=sprockets --skip-git -m <app_template.rb URL>` with `THECORE_APP_TEMPLATE_NON_INTERACTIVE=1 THECORE_APP_TEMPLATE_RUN_INSTALLERS=true` (thecore_generators#23) — the two env vars together preserve this command's own long-standing zero-prompt, always-installs-everything contract exactly, with no regression. `--skip-git` is kept deliberately: the App template itself never touches git (verified against its source — no `git_init`/`git`/`run "git ..."` calls anywhere in `app_template.rb`), so this command's own git handling is entirely undelegated, unchanged in spirit from before.

What stays JS-side, because the App template doesn't own it: `ctx.write.gitignoreFile` (a much richer `.gitignore` than Rails' own default — OS/editor noise, secrets, `vendor/bundle`, `config/database.yml` — the template writes none of this), `config/sidekiq.yml` (queue names derived from `COMPOSE_PROJECT_NAME`), the `version` file (seeded `"3.0.1"`), a `config/environments/development.rb` patch (`raise_on_missing_callback_actions` off), `rails db:drop ; db:create && db:migrate && thecore:db:seed`, `vendor/custombuilds`/`vendor/deploytargets` (`vendor/submodules` is **not** duplicated here — the template already creates it), `.dockerignore` removal, and finally `git init && git checkout -b main && git add . && git commit -m "Initial commit" && git branch -M main`. That commit is now a **single** one, not the old two-commit "Initial commit" / "Add Thecore 3 gems and configuration" split — delegation means the Gemfile/CI content is already complete by the time git ever runs (the old split existed because gems used to be staged into the Gemfile in three separate `fs.writeFileSync` passes across the two commits; that staging no longer happens), matching `thecore:atom`'s own "one initial commit" precedent (ADR 0006).

No `thecore_generators` Gemfile guard here (see below) — `createApp` bootstraps a brand-new app with no Gemfile to check yet, and the App template adds `thecore_generators` as a dependency unconditionally as part of its own Gemfile stack.
...
- `insertGemIntoDevelopmentGroup(gemfileContent, gemLine)` — pure content transform (no fs I/O); inserts `gemLine` into an existing bare `group :development do ... end` block or creates one at the end of the file. Used by `libs/thecoreGeneratorsGuard.js` to add `thecore_generators` as a dev-only dependency (`createApp.js` no longer calls this directly since its own delegation — the App template's own Gemfile stack adds `thecore_generators` itself now).
